...
var WfMultiselectDropdown = /** @class */ (function () {
    function WfMultiselectDropdown(hostRef) {
    ...
    WfMultiselectDropdown.prototype.handleInput = function (e) {
        var _a = this, multiselectDropdownOptions = _a.multiselectDropdownOptions, selectedOptions = _a.selectedOptions;
    ...
    WfMultiselectDropdown.prototype.handleDropdownChange = function (e) {
        e.stopPropagation();
        if (e.detail) {
    ...
    WfMultiselectDropdown.prototype.handleKeyDown = function (e) {
        ...
        var optionClass = 'multiselect-dropdown-option';
        var focusClass = 'multiselect-dropdown-option-focused';
        var optionsList = host.shadowRoot.querySelector(".multiselect-dropdown-options-wrapper");
        if (optionsList) {
            var focusedElToClick = optionsList.querySelector(".multiselect-dropdown-option-focused .multiselect-dropdown-option-checkbox");
    ...
    WfMultiselectDropdown.prototype.handleValueChange = function (newValue, oldValue) {
        var _this = this;
        var validateValue = function (value) { return Array.isArray(value) && _this.validateValues(value); };
        if (!validateValue(newValue)) {
            this.value = validateValue(oldValue) ? oldValue : [];
            throw new Error('Value is not valid');
        }
    };
    WfMultiselectDropdown.prototype.validateValues = function (value) {
        var countUpdatedOptions = 0;
        var mappedOptions = this.multiselectDropdownOptions.map(function (option) {
            var checked = value && value.includes(option.value);
            if (checked) {
                countUpdatedOptions++;
            }
    ...
    WfMultiselectDropdown.prototype.toggleInput = function (opened) {
        if (opened) {
    ...
    WfMultiselectDropdown.prototype.handleSave = function () {
        var _a = this, selectedOptions = _a.selectedOptions, field = _a.field;
        this.value = __spreadArrays(selectedOptions);
        this.change.emit({
            field: field,
    ...
    WfMultiselectDropdown.prototype.closeFilter = function (shouldSave) {
    ...
    WfMultiselectDropdown.prototype.handleCheckboxChange = function (option, event) {
        var selectedOptions = this.selectedOptions;
        var checked = !!event ? event.detail : !!!option.selected;
        if (!!event) {
            event.stopPropagation();
        }
        this.multiselectDropdownOptions = this.multiselectDropdownOptions.map(function (item) { return item === option ? Object.assign(Object.assign({}, option), { selected: checked }) : item; });
        if (checked) {
    ...
    WfMultiselectDropdown.prototype.handleSelectAll = function (event) {
        var selected = event instanceof CustomEvent ? event.detail : event;
        this.multiselectDropdownOptions = this.multiselectDropdownOptions.map(function (option) {
            option.selected = selected;
            return option;
        });
        this.updateSelectedOptions();
    };
    WfMultiselectDropdown.prototype.handleSelectAllClick = function (allSelected) {
        this.handleSelectAll(!allSelected);
    };
    WfMultiselectDropdown.prototype.handleCheckboxClick = function (option) {
        this.handleCheckboxChange(option);
    };
    WfMultiselectDropdown.prototype.updateSelectedOptions = function () {
        var multiselectDropdownOptions = this.multiselectDropdownOptions;
        this.selectedOptions = multiselectDropdownOptions
            .filter(function (option) { return option.selected; })
            .map(function (option) { return option.value; });
    };
    WfMultiselectDropdown.prototype.getDisplayValue = function (option) {
        var displayItem = this.multiselectDropdownOptions.find(function (originalOption) { return originalOption.value === option; });
        return !!displayItem.label ? displayItem.label : displayItem.value;
    };
    WfMultiselectDropdown.prototype.showSelectedOptions = function () {
        var _this = this;
        var _a = this, selectedOptionsElement = _a.selectedOptionsElement, selectedOptions = _a.selectedOptions, multiselectDropdownOptions = _a.multiselectDropdownOptions, optionsVisibilityLimit = _a.optionsVisibilityLimit;
        ...
        var limit = !!optionsVisibilityLimit
            ? typeof optionsVisibilityLimit === 'string'
                ? parseInt(optionsVisibilityLimit)
                : optionsVisibilityLimit
            : selectedOptions.length;
        var moreOptionsInfo = limit < selectedOptions.length ? " & " + (selectedOptions.length - limit) + " more..." : '';
        this.initialValue = selectedOptionsElement.value = "" + selectedOptions
            .slice(0, limit)
            .map(function (option) { return _this.getDisplayValue(option); })
            .join(', ') + moreOptionsInfo;
    };
    WfMultiselectDropdown.prototype.getSortableValue = function (option) {
        return !!option.label
            ? option.label.toString().toLowerCase()
            : option.value.toString().toLowerCase();
    };
    WfMultiselectDropdown.prototype.sortOptions = function (options) {
        var _this = this;
        return options.sort(function (a, b) {
            var x = _this.getSortableValue(a);
            var y = _this.getSortableValue(b);
            return x < y ? -1 : x > y ? 1 : 0;
        });
    };
    WfMultiselectDropdown.prototype.filterUniqueOptions = function (options) {
        return options.filter(function (option, index) { return options.findIndex(function (a) { return a.value === option.value; }) === index; });
    };
    WfMultiselectDropdown.prototype.componentWillLoad = function () {
        this.handleSave = this.handleSave.bind(this);
        this.handleSelectAll = this.handleSelectAll.bind(this);
        var options = this.options;
        this.handleOptionsUpdate(typeof options === 'string' ? JSON.parse(options) : options);
    };
    WfMultiselectDropdown.prototype.handleOptionsUpdate = function (newValue, oldValue) {
        if (!Array.isArray(newValue) ||
            newValue.some(function (item) { return typeof item !== 'object' || !item.hasOwnProperty('value'); })) {
    ...
    WfMultiselectDropdown.prototype.componentDidLoad = function () {
        this.showSelectedOptions();
    };
    WfMultiselectDropdown.prototype.findSearchStrInValue = function (searchStr, value) {
    ...
    WfMultiselectDropdown.prototype.renderMultiselectDropdownOptions = function () {
        var _this = this;
        var _a = this, multiselectDropdownOptions = _a.multiselectDropdownOptions, searchStr = _a.searchStr, checkboxSize = _a.checkboxSize;
        var filteredOptions = multiselectDropdownOptions.filter(function (option) { return _this.findSearchStrInValue(searchStr, !!option.label ? option.label : option.value); });
        var filteredSelectedOptions = filteredOptions.filter(function (option) { return option.selected; });
        var allSelected = filteredOptions.length === filteredSelectedOptions.length;
        ...
        var optionAttributes = function (option) {
            var _a;
            var classes = (_a = {},
                _a['multiselect-dropdown-option-checkbox'] = true,
                _a["multiselect-dropdown-option-checkbox-selected"] = !!option.selected,
                _a);
            return {
                class: classes,
                size: checkboxSize,
        ...
        return (h("div", { class: "multiselect-dropdown-options-wrapper" }, filteredOptions.length === 0 ? (h("div", { class: "multiselect-dropdown-option-not-found" }, "Cannot be found")) : (h("div", { class: "multiselect-dropdown-options-list" }, searchStr.length === 0 && (h("div", { class: "multiselect-dropdown-option multiselect-dropdown-select-all" }, h("wf-checkbox", Object.assign({}, selectAllAttributes, { onClick: function () { return _this.handleSelectAllClick(allSelected); }, onChange: stopPropagation })))), filteredOptions.map(function (option) { return (h("div", { class: "multiselect-dropdown-option" }, h("wf-checkbox", Object.assign({}, optionAttributes(option), { onClick: function () { return _this.handleCheckboxClick(option); }, onChange: stopPropagation })))); })))));
    };
    WfMultiselectDropdown.prototype.render = function () {
        var _a;
        var _this = this;
        var _b = this, label = _b.label, variant = _b.variant, size = _b.size, buttonSize = _b.buttonSize, required = _b.required, error = _b.error, errorMessage = _b.errorMessage, placeholder = _b.placeholder, disabled = _b.disabled;
        ...
        return (h("div", { class: "dropdown-multiselect-dropdown" }, h("div", { class: "dropdown-trigger" }, !!label ? (h("label", { "data-dropdown-trigger": true, class: "form-label" }, label, !!error || (!!required && h("span", { class: asteriskClasses }, "*")))) : null, h("wf-input", Object.assign({ "data-dropdown": DropdownTriggerType.OPEN_ONLY_WITH_ICON, icon: "wf-arrow-down", class: "filter-selected-options" }, inputAttributes, { ref: function (el) { return (_this.selectedOptionsElement = el); } }))), h("wf-dropdown", { ref: function (el) { return (_this.dropdown = el); }, "scroll-sensitive": "false", disabled: disabled, adjustHeight: true }, h("div", { class: "multiselect-dropdown" }, this.renderMultiselectDropdownOptions(), h("div", { class: "multiselect-dropdown-footer" }, h("wf-button", { onClick: function () {
                _this.closeFilter();
            }, variant: "link", size: buttonSize }, "Cancel"), h("wf-button", { onClick: this.handleSave, size: buttonSize }, "Apply"))))));
    };
    Object.defineProperty(WfMultiselectDropdown.prototype, "host", {
        get: function () { return getElement(this); },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(WfMultiselectDropdown, "watchers", {
        get: function () {
            return {
    ...
    Object.defineProperty(WfMultiselectDropdown, "style", {
