...
var PluginManager = {
    ...
    pluginEvent: function pluginEvent(eventName, sortable, evt) {
        ...
        plugins.forEach(function (plugin) {
            if (!sortable[plugin.pluginName])
                return; // Fire global events if it exists in this sortable
    ...
    modifyOption: function modifyOption(sortable, name, value) {
        ...
        plugins.forEach(function (plugin) {
            ...
            if (!sortable[plugin.pluginName])
                return; // If static option listener exists for this option, call in the context of the Sortable's instance of this plugin
...
var pluginEvent = function pluginEvent(eventName, sortable) {
    var _ref = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {}, originalEvent = _ref.evt, data = _objectWithoutProperties(_ref, ["evt"]);
...
Sortable.prototype =
    /** @lends Sortable.prototype */
    {
        ...
        evt) {
            if (tapEvt) {
                var options = this.options, fallbackTolerance = options.fallbackTolerance, fallbackOffset = options.fallbackOffset, touch = evt.touches ? evt.touches[0] : evt, ghostMatrix = ghostEl && matrix(ghostEl), scaleX = ghostEl && ghostMatrix && ghostMatrix.a, scaleY = ghostEl && ghostMatrix && ghostMatrix.d, relativeScrollOffset = PositionGhostAbsolutely && ghostRelativeParent && getRelativeScrollOffset(ghostRelativeParent), dx = (touch.clientX - tapEvt.clientX + fallbackOffset.x) / (scaleX || 1) + (relativeScrollOffset ? relativeScrollOffset[0] - ghostRelativeParentInitialScroll[0] : 0) / (scaleX || 1), dy = (touch.clientY - tapEvt.clientY + fallbackOffset.y) / (scaleY || 1) + (relativeScrollOffset ? relativeScrollOffset[1] - ghostRelativeParentInitialScroll[1] : 0) / (scaleY || 1); // only set the status to dragging, when we are actually dragging
...
var DragManager = /** @class */ (function () {
    function DragManager(dragClass) {
        Sortable.mount(new AutoScrollPlugin());
        this.dragIcon = "." + dragClass;
    }
    DragManager.prototype.initContainer = function (dragArea) {
        this.dragManager = new Sortable(dragArea, {
            handle: this.dragIcon,
        });
        this.initOrder = this.getCurrentOrder();
    };
    DragManager.prototype.resetDraggable = function () {
        if (this.dragManager)
            this.dragManager.sort(this.initOrder);
    };
    DragManager.prototype.unload = function () {
        if (this.dragManager)
            this.dragManager.destroy();
    };
    DragManager.prototype.getCurrentOrder = function () {
        return this.dragManager.toArray();
    };
    return DragManager;
}());
export { DragManager as D };
