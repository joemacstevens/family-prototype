...
function getAllNodes(host, selector) {
...
function setPX(number) {
    return number + "px";
}
function showErrorMessage(error, errorMessage, type) {
    var messageType = !!type ? type : 'text';
    return (!!error && !!errorMessage && errorMessage[messageType] && errorMessage[messageType].length > 0);
}
function prepareErrorTooltip(error, errorMessage) {
    var defaultType = 'error';
    var defaultEvent = 'none';
...
function setupTimeout(callback, time) {
...
function parseArrayProperty(property) {
    ...
    return lodash_clonedeep(property);
}
function animate(element, start, end, prop, duration) {
    var _a;
    if (prop === void 0) { prop = 'right'; }
    if (duration === void 0) { duration = 500; }
    element.animate((_a = {},
        _a[prop] = [start, end],
        _a), { duration: duration, fill: 'forwards' });
}
function stopPropagation(e) {
    e.stopPropagation();
}
export { showErrorMessage as a, prepareErrorTooltip as b, setupTimeout as c, detectIE as d, lodash_clonedeep as e, getAllNodes as f, generateUniqueId as g, setPX as h, animate as i, parseArrayProperty as p, removePX as r, stopPropagation as s };
