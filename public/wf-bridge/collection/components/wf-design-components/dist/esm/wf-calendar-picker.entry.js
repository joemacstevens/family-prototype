...
/*
 0BSD
*/
(function(F,y){module.exports=y();})(commonjsGlobal,function(){function F(a,c,b){b=b||[];if(a instanceof Element)c.apply(c,[a].concat(b));else{var d;var e=a.length;for(d=0;d<e;++d)c.apply(c,[a[d]].concat(b));}}function y(a){F(a,function(a){a.parentElement.removeChild(a);});}function J(a,c){do a=a.parentElement;while(a&&!B(a,c));return a}function B(a,c){return (a.matches||a.webkitMatchesSelector||a.msMatchesSelector).call(a,
...
const CalendarPicker = class {
    ...
    render() {
        ...
        return (h("span", null, h("div", { class: "dropdown-trigger" }, !!label ? (h("label", { "data-dropdown-trigger": true, class: "form-label" }, label, (!!error || !!required) && h("span", { class: asteriskClasses }, "*"))) : null, h("wf-input", Object.assign({ ref: (el) => (this.inputElement = el) }, inputAttributes))), h("wf-dropdown", { ref: (el) => (this.dropdown = el), alignWithParent: false, disabled: disabled }, range ? (h("div", { class: "date-picker-content" }, showPreset && (h("ul", { class: "date-picker-list" }, presets.map((preset) => this.renderPresetItem(preset)))), h("div", { class: "date-picker-calendar-control" }, h("div", { class: "data-picker-range" }, h("wf-input", { ref: (el) => (this.fromInputElement = el), label: "From", size: inputSize }), h("wf-input", { ref: (el) => (this.toInputElement = el), label: "To", size: inputSize })), h("div", { class: "date-picker-range", ref: (el) => (this.wrapperElement = el) }), h("div", { class: "date-picker-footer" }, h("wf-button", { onClick: () => this.closeDropdown(), variant: "link", size: buttonSize }, "Cancel"), h("wf-button", { onClick: this.onApplyClick, size: buttonSize }, "Apply"))))) : (h("div", { class: wrapperClasses, ref: (el) => (this.wrapperElement = el) })))));
    }
