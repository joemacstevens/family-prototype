...
const dragManager = require('./drag-manager-dadf748b.js');
const keyboardControl = require('./keyboard-control-b991dc46.js');
...
const WfColumnsCustomizer = class {
    constructor(hostRef) {
    ...
    componentWillLoad() {
        this.getColumnCopy();
        this.dragManager = new dragManager.DragManager('drag-handle');
    }
    componentDidRender() {
    ...
    getColumnCopy() {
        ...
        this.editedColumns = utils.parseArrayProperty(defaultColumns);
    }
    getHost() {
        return this.host;
    }
    ...
    handleKeyDown(e) {
        ...
        if (!dropdown.opened)
            return;
        e.cancelBubble = true;
        if (e.key === types.KeyValue.ESC_KEY) {
            e.preventDefault();
        ...
        if (optionsList) {
            keyboardControl.handleKeyboardControl(e, optionClass, focusClass, optionsList);
        }
    }
    rerenderListOrder() {
    ...
    renderLockedColumn(column) {
        ...
        return (core.h("li", { class: columnClasses }, core.h("wf-switch", { label: column.headerName, disabled: true, "icon-placement": "left" })));
    }
    renderEditableColumn(column, index) {
        ...
        return (core.h("li", { "data-value": index, class: columnClasses }, core.h("wf-icon", { class: "columns-item-icon drag-handle", name: this.icon, size: this.iconSize }), core.h("span", null, column.headerName), !column.lockVisible ? (core.h("wf-switch", { class: "columns-item-switch", onChange: this.handleSwitchChange.bind(this, column), onWfChange: utils.stopPropagation, checked: !column.hide })) : ('')));
    }
    ...
    renderFilteredColumnsList() {
        ...
        return filteredColumns.length === 0 ? (core.h("li", { class: "not-found" }, "Column not found")) : (filteredColumns.map((column, index) => {
            if (index >= maxSearchItems)
                return;
            return (core.h("li", { class: "column-customizer-search-list-item form-select-option form-select-option-lg", onClick: (event) => this.handleSetSelectedColumn(column, event) }, column.headerName));
        }));
    }
    render() {
        ...
        return opened ? (core.h("wf-drawer", { class: "column-customizer", header: header, width: "slim", opened: opened }, core.h("div", { slot: "header" }, core.h("slot", null), enableSearch ? (core.h("div", { class: "dropdown-column-customizer form-control-wrapper" }, core.h("div", { class: "dropdown-trigger", "data-dropdown": true }, core.h("wf-input", Object.assign({ "data-dropdown-trigger": true }, searchInputAttributes))), core.h("wf-dropdown", { ref: (el) => (this.dropdown = el), "scroll-sensitive": "false" }, core.h("ul", { class: "column-customizer-search-list form-select-list" }, this.renderFilteredColumnsList())))) : null), !!lockedColumns.length && (core.h("ul", { class: "columns locked" }, lockedColumns.map(this.renderLockedColumn))), !!editableColumns.length && (core.h("ul", { class: "drag-area columns" }, editableColumns.map(this.renderEditableColumn))), core.h("div", { slot: "footer" }, core.h("wf-button", { class: "column-customizer-footer-button", onClick: this.handleOnCancelClick, variant: "secondary", size: buttonSize }, "Cancel"), core.h("wf-button", { class: "column-customizer-footer-button", onClick: this.handleOnSaveClick, size: buttonSize }, "Save")))) : (core.h("div", { class: "column-customizer-hidden" }, core.h("slot", null)));
    }
...
exports.wf_columns_customizer = WfColumnsCustomizer;
