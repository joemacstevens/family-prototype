...
const WfAmountFilter = class {
    ...
    handleKeyPress(event) {
        if (event.keyCode === types.KeyCodes.ENTER_KEY) {
    ...
    renderListItems() {
        ...
        return Object.keys(filterListItems).map((item) => (core.h("li", { class: `date-picker-list-item ${internalSelectedTab === item ? 'active' : ''}`, onClick: () => {
    ...
    render() {
        ...
        const innerInputAttributes = Object.assign(Object.assign(Object.assign(Object.assign({ class: 'amount-filter-value', variant, type: inputType, label: innerLabel, size: inputSize, error: !!inputError, errorMessage,
            maskOptions }, (!!maskOptions && inputType === 'masked' && { maskValue })), (!!prefixLabel && { prefixLabel })), (!!textAlign && { textAlign })), { onInput: this.handleInputChange, onKeyDown: this.handleKeyPress, value: !!value || value === 0 ? value.toString() : '', onChange: utils.stopPropagation, onWfChange: utils.stopPropagation });
        return (core.h("div", { class: "amount-filter" }, core.h("div", { class: "dropdown-trigger" }, !!label ? (core.h("label", { "data-dropdown-trigger": true, class: "form-label" }, label, (!!error || !!required) && core.h("span", { class: asteriskClasses }, "*"))) : null, core.h("wf-input", Object.assign({ ref: (el) => (this.triggerInput = el) }, inputAttributes))), core.h("wf-dropdown", { ref: (el) => (this.dropdown = el), alignWithParent: false, disabled: disabled }, core.h("div", { class: "date-picker-content" }, core.h("ul", { class: "date-picker-list" }, this.renderListItems()), core.h("div", { class: "date-picker-calendar-control" }, core.h("div", { class: "date-picker-range" }, core.h("wf-input", Object.assign({ ref: (el) => (this.innerInput = el) }, innerInputAttributes))), core.h("div", { class: "date-picker-footer" }, core.h("wf-button", { variant: "link", onClick: this.cancelFilter, size: buttonSize }, "Cancel"), core.h("wf-button", { onClick: this.handleApplyButtonClick, size: buttonSize }, "Apply")))))));
    }
...
exports.wf_amount_filter = WfAmountFilter;
