...
const index = require('./index-37c94e48.js');
...
const WfSelect = class {
    constructor(hostRef) {
        ...
        /** Select ID */
        this.selectId = utils.generateUniqueId();
    ...
    handleKeyDown(ev) {
        ...
        if (!dropdown.opened) {
            if (ev.key === types.KeyValue.ENTER_KEY) {
                ev.preventDefault();
        ...
        if (ev.key === types.KeyValue.ESC_KEY) {
            ev.preventDefault();
        ...
        keyboardControl.handleKeyboardControl(ev, optionClass, focusClass, optionsList);
    }
    handleValueChange(newValue, oldValue) {
        ...
        if (selectOptions) {
            const index$1 = selectOptions.findIndex((option) => index.isEqual(option.value, newValue));
            if (index$1 === -1) {
                this.value = oldValue;
                throw new Error('Value is not valid');
            }
            else {
                selectOptions[index$1].selected = true;
                this.selectedValue = selectOptions[index$1].selectedName || selectOptions[index$1].name;
    ...
    updateSelectOptions(options, initial = false) {
        ...
        this.selectOptions = options
            ...
            : utils.getAllNodes(host, 'wf-select-option, brml-select-option').map(parseOptionElement);
    ...
    renderOption(option, index) {
        ...
        return (core.h("li", Object.assign({ class: optionClass }, optionProps, { onClick: () => handleSelectChange(option) }), !this.customRow ? core.h("a", null, " ", option.name) : core.h("div", { innerHTML: this.customRow(option) })));
    }
    renderListOptions(options) {
        ...
        return (core.h("wf-dropdown", { ref: (el) => (this.dropdown = el), class: "form-control-select-dropdown", disabled: disabled, parentSelector: ".form-group" }, core.h("ul", { tabindex: "-1", class: optionListClass }, filteredOption.map((option, index) => this.renderOption(option, index)), core.h("slot", { name: "footer" }))));
    }
    renderLabel() {
        ...
        return (core.h("label", { "data-dropdown-trigger": true, class: labelClasses, htmlFor: selectId }, currentLabel, (!!error || !!required) && core.h("span", { class: asteriskClasses }, "*")));
    }
    renderButtonTrigger() {
        ...
        return (core.h("wf-button", Object.assign({}, buttonAttributes, { ref: (el) => (this.selectButton = el), iconPlacement: "right" }), placeholder));
    }
    renderInputTrigger() {
        ...
        return core.h("wf-input", Object.assign({}, inputAttributes, { ref: (el) => (this.selectInput = el) }));
    }
    render() {
