...
const WfRadio = class {
    constructor(hostRef) {
        ...
        this.getSelectedOptionIndex = () => {
            const index$1 = this.radioOptions.findIndex((option) => index.isEqual(this.value, option.value));
            return index$1 !== -1 ? index$1 : 0;
        };
    ...
    valueWatcher(newValue, oldValue) {
        if (newValue === undefined)
            return;
        const optionByValue = this.radioOptions.find((option) => index.isEqual(newValue, option.value));
        if (optionByValue)
            return;
    ...
    handleKeyDown(e) {
        if (e.key === types.KeyValue.ARROW_DOWN || e.key === types.KeyValue.ARROW_RIGHT) {
            e.preventDefault();
        ...
        else if (e.key === types.KeyValue.ARROW_UP || e.key === types.KeyValue.ARROW_LEFT) {
            e.preventDefault();
            if (this.disabled || this.allOptionsDisabled())
                return;
    ...
    componentWillLoad() {
        ...
        this.radioName = name || utils.generateUniqueId();
    ...
    updateRadioOptions(options) {
        ...
        this.radioOptions = options
            ? options
            : utils.getAllNodes(host, 'wf-radio-option, brml-radio-option').map(parseOptionElement);
    ...
    renderOption(option, index$1) {
        ...
        const labelClass = {
            ...
            ['checked']: index.isEqual(this.value, option.value),
        };
        const checkClasses = {
            'form-check': true,
        ...
        if (!option.optionId) {
            option.optionId = utils.generateUniqueId();
        }
        const optionProps = {
            type: 'radio',
            name: radioName,
        ...
        return (core.h("div", { class: checkClasses }, core.h("input", Object.assign({}, optionProps)), core.h("button", Object.assign({}, buttonProps), option.label)));
    }
    render() {
        ...
        return (core.h("wf-tooltip", Object.assign({}, utils.prepareErrorTooltip(error, errorMessage)), core.h("div", { class: "form-group" }, !!label && (core.h("label", { class: "form-label" }, label, (!!error || !!required) && core.h("span", { class: asteriskClasses }, "*"))), radioOptions && radioOptions.map((option, index) => this.renderOption(option, index)), utils.showErrorMessage(error, errorMessage) && (core.h("div", { class: "form-control-error-message", innerHTML: errorMessage.text })))));
    }
