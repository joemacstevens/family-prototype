...
var pickmeup_min = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 0BSD
*/
(function(F,y){module.exports=y();})(_commonjsHelpers.commonjsGlobal,function(){function F(a,c,b){b=b||[];if(a instanceof Element)c.apply(c,[a].concat(b));else{var d;var e=a.length;for(d=0;d<e;++d)c.apply(c,[a[d]].concat(b));}}function y(a){F(a,function(a){a.parentElement.removeChild(a);});}function J(a,c){do a=a.parentElement;while(a&&!B(a,c));return a}function B(a,c){return (a.matches||a.webkitMatchesSelector||a.msMatchesSelector).call(a,
...
const CalendarPicker = class {
    constructor(hostRef) {
        ...
        this.onApplyClick = () => {
            this.date = pickmeup_min(this.wrapperElement).get_date(true);
    ...
    handleDateChange(newDate) {
        ...
        if (!range) {
            const date = hooks(newDate, format.toUpperCase());
    ...
    handleUserInput(event) {
        ...
        event.stopPropagation();
        const fromData = hooks(fromInputElement.value, format.toUpperCase());
        const toData = hooks(toInputElement.value, format.toUpperCase());
    ...
    handleFocusedInput(event) {
        if (event.key === types.KeyValue.ENTER_KEY) {
        ...
        pickmeup_min(wrapperElement, {
            ...
            render: (date) => {
                if (this.isDateDisabled(date)) {
                    const currentRange = pickmeup_min(wrapperElement).get_date();
    ...
    initBlackoutDate() {
        ...
        holidays.forEach((holiday) => {
            if (!Array.isArray(holiday)) {
                const date = hooks(holiday, format.toUpperCase());
                if (date.isValid())
                    blackoutDates.push(date);
            }
            else {
                const fromDate = hooks(holiday[0], format.toUpperCase());
                const toDate = hooks(holiday[1], format.toUpperCase());
                if (this.validateDate(fromDate, toDate))
                    for (let date = hooks(fromDate); date.diff(toDate, 'days') <= 0; date.add(1, 'days'))
    ...
    applySelectedRange() {
        const formattedDate = pickmeup_min(this.wrapperElement).get_date(true);
        const date = pickmeup_min(this.wrapperElement).get_date(false);
    ...
    updateDateInput(from, to) {
        ...
        const currentData = pickmeup_min(wrapperElement).get_date(true);
    ...
    renderPresetItem(preset) {
        ...
        return (!preset.hidden && (core.h("li", { class: `date-picker-list-item ${activePreset.name === preset.name ? 'active' : ''}`, onClick: () => {
    ...
    render() {
        ...
        const inputAttributes = {
            readonly: true,
            'data-dropdown': types.DropdownTriggerType.TOGGLE,
            ...
            disabled,
            onChange: utils.stopPropagation,
        };
        ...
        return (core.h("span", null, core.h("div", { class: "dropdown-trigger" }, !!label ? (core.h("label", { "data-dropdown-trigger": true, class: "form-label" }, label, (!!error || !!required) && core.h("span", { class: asteriskClasses }, "*"))) : null, core.h("wf-input", Object.assign({ ref: (el) => (this.inputElement = el) }, inputAttributes))), core.h("wf-dropdown", { ref: (el) => (this.dropdown = el), alignWithParent: false, disabled: disabled }, range ? (core.h("div", { class: "date-picker-content" }, showPreset && (core.h("ul", { class: "date-picker-list" }, presets.map((preset) => this.renderPresetItem(preset)))), core.h("div", { class: "date-picker-calendar-control" }, core.h("div", { class: "data-picker-range" }, core.h("wf-input", { ref: (el) => (this.fromInputElement = el), label: "From", size: inputSize }), core.h("wf-input", { ref: (el) => (this.toInputElement = el), label: "To", size: inputSize })), core.h("div", { class: "date-picker-range", ref: (el) => (this.wrapperElement = el) }), core.h("div", { class: "date-picker-footer" }, core.h("wf-button", { onClick: () => this.closeDropdown(), variant: "link", size: buttonSize }, "Cancel"), core.h("wf-button", { onClick: this.onApplyClick, size: buttonSize }, "Apply"))))) : (core.h("div", { class: wrapperClasses, ref: (el) => (this.wrapperElement = el) })))));
    }
